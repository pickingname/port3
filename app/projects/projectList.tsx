...
const projects: Project[] = [
  ...
  { id: 6, name: "zinc", year: 2024, status: 'archive' },
]
          ...
          <table className="w-full">
            <thead>
              <tr className="dark:bg-neutral-700 bg-neutral-200 dark:text-white text-black">
                ...
                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider cursor-pointer" onClick={() => toggleSort('year')}>
                  Year <SortIcon field="year" />
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider cursor-pointer" onClick={() => toggleSort('status')}>
            ...
            <tbody className="divide-y dark:divide-neutral-700">
              {sortedProjects.map((project) => (
                <tr key={project.id} className="hover:bg-neutral-500/30 dark:hover:bg-neutral-300/30 transition-colors ease-out duration-200">
                  <td className="px-6 py-4 whitespace-nowrap">{project.name}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{project.year}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-medium rounded-sm ${
                      project.status === 'active' ? 'bg-green-100 text-green-800' :
                      project.status === 'stale' ? 'bg-yellow-100 text-yellow-800' :
